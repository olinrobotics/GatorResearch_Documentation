...
\begin{enumerate}
\item Potomac Electric DC motor, which is an aftermarket power steering assist motor purchased by the 2009-2010 SCOPE team
\item Advanced Motor Controls 30A20AC analog servo drive with filter card
\item National Instruments NI9263 chassis that sends +/- 10V control signals to the analog servo drive
\end{enumerate}

\noindent In the LabVIEW code, the steering controller code communicates with the NI9263 module and uses the NI9263 module to output control signals to the 30A20AC analog servo drive. Based on those control signals, the analog servo drive will then pass the appropriate motor voltage through the filter card before passing those signals to the Potomac Electric DC Motor mounted below the steering wheel.